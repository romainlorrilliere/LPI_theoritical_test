...
d_pop[,sp := as.integer(as.character((sp)))]
...
```{r}
gg <- ggplot(d_compare,aes(x=dt_mean,y=i,colour=as.factor(N0K))) + facet_wrap(K~r,scales="free")
gg <- gg + geom_abline(slope = 1,intercept=0,colour="orange",alpha=.7,linewidth=1.1)
gg <- gg + geom_point()
print(gg)
...
```{r, eval = FALSE}
gg <- ggplot(d_compare,aes(x=dt_md,y=i,colour=N0)) + facet_wrap(K~r,scales="free")
gg <- gg + geom_abline(slope = 1,intercept=0,colour="orange",alpha=.7,linewidth=1.1)
gg <- gg + geom_point()
print(gg)
...
```{r, eval = FALSE}
gg <- ggplot(d_compare,aes(x=dt_glm,y=i,colour=N0)) + facet_wrap(K~r,scales="free")
gg <- gg + geom_abline(slope = 1,intercept=0,colour="orange",alpha=.7,linewidth=1.1)
gg <- gg + geom_point()
print(gg)
